...
try:
    ...
    # Minimal Translator fallback: returns the input string (no-op translation)
    class Translator:
    ...
    def humanize_list(seq, conjunction="and"):
        # Simple humanize implementation for tests
        items = [str(i) for i in seq]
        if not items:
            return ""
        if len(items) == 1:
            return items[0]
    ...
    def pagify(text: str, delims=None, page_length: int = 200):
        # Very small pager used in a few helpers; return whole text if short
        if text is None:
            return []
        text = str(text)
        if len(text) <= page_length:
            return [text]
        # naive chunking by fixed page_length
...
# Minimal development/owner list for environments that don't set bot owner IDs.
# Some modules import DEV_LIST to check developer privileges; provide a safe default.
DEV_LIST: list = []

# (Other legacy constants like RARITIES are available via the Rarities enum.)
...
class HeroClasses(Enum):
    ...
    def desc(self):
        # Short descriptions for each hero class. Keep these brief to avoid
        # very long multiline literals which can be fragile in tests.
        return {
            "hero": _("Your basic adventuring hero."),
            "wizard": _("Wizards focus to boost magic; use focus during attacks."),
            "tinkerer": _("Tinkerers craft devices by combining items."),
            "berserker": _("Berserkers excel in raw melee damage."),
            "cleric": _("Clerics support allies with heals and buffs."),
            "ranger": _("Rangers are versatile fighters with ranged skill."),
            "bard": _("Bards inspire allies and debuff foes."),
            "psychic": _("Psychics use mind powers to control the field."),
        }
