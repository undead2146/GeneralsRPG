...
# Red imports are optional for running unit tests outside of Red. Provide
# minimal, safe fallbacks when Red is not available.
try:
    ...
    class Cog:  # very small sentinel base type for typing/imports
        pass

    class Context:  # minimal placeholder used for type hints only
        bot = None
        guild = None
        channel = None
        author = None
    ...
    def check(predicate):
        # No-op decorator replacement for Red's check; returns the function unchanged.
        def decorator(func):
            return func
...
def escape(t: str) -> str:
    text = filter_various_mentions(t)
    try:
        # Some test stubs provide a simplified `escape` that doesn't accept
        # the `mass_mentions`/`formatting` keyword arguments.
        return _escape(text, mass_mentions=True, formatting=True)
    except TypeError:
        # Fallback to call without kwargs for compatibility with minimal stubs
        return _escape(text)
...
class UnitActionView(discord.ui.View):
    """A small view with Heal and Repair buttons for a single unit-owner.

    The view calls back into the Adventure cog to perform the action. It
    restricts interactions to the provided author (the unit owner).
    """

    def __init__(self, timeout: float, author: discord.User, cog: object, target_user: discord.Member, unit_id: str = None):
        super().__init__(timeout=timeout)
        self.author = author
        self.cog = cog
    ...
    @discord.ui.button(label=_("Heal"), style=discord.ButtonStyle.green)
    async def heal_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if interaction.user.id != self.author.id:
            await interaction.response.send_message(_("You are not authorized to interact with this."), ephemeral=True)
            return
        # Delegate to the cog handler
        try:
            await self.cog._handle_unit_action(interaction, "heal", self.target_user, self.unit_id)
        except Exception:
            await interaction.followup.send(_("Failed to perform heal."), ephemeral=True)
        self.stop()

    @discord.ui.button(label=_("Repair"), style=discord.ButtonStyle.blurple)
    async def repair_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if interaction.user.id != self.author.id:
            ...
            return
        try:
            # default repair action: prefer kits then parts handled by cog
            await self.cog._handle_unit_action(interaction, "repair", self.target_user, self.unit_id)
        except Exception:
            await interaction.followup.send(_("Failed to perform repair."), ephemeral=True)
        self.stop()

    @discord.ui.button(label=_("Repair (Kit)"), style=discord.ButtonStyle.blurple, row=1)
    async def repair_kit_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        if interaction.user.id != self.author.id:
            await interaction.response.send_message(_("You are not authorized to interact with this."), ephemeral=True)
            return
    ...
    @discord.ui.button(label=_("Repair (Parts)"), style=discord.ButtonStyle.grey, row=1)
    async def repair_parts_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        if interaction.user.id != self.author.id:
            await interaction.response.send_message(_("You are not authorized to interact with this."), ephemeral=True)
            return
        try:
            # pass explicit hint via unit_id param None; cog will check backpack parts when processing
            await self.cog._handle_unit_action(interaction, "repair", self.target_user, self.unit_id)
        except Exception:
            await interaction.followup.send(_("Failed to perform repair."), ephemeral=True)
        self.stop()
