...
import discord
# redbot may not be available in the test environment. Provide minimal,
# import-time-safe fallbacks for the specific bits we need so test collection
# doesn't fail. Keep these tiny and behaviour-preserving for tests.
try:
    from redbot.core import commands
except Exception:  # pragma: no cover - test shim
    class _CmdShim:
        class Cog:  # lightweight placeholder
            pass

        class Context:  # lightweight placeholder
            pass
...
try:
    ...
    # Translator(...) should return a callable that formats/returns messages.
    class Translator:
...
try:
    ...
    def pagify(text, delims=None, priority=False, page_length=1900):
        # yield the whole text as a single page
        yield text

from adventure.bank import bank
from adventure.charsheet import Character, Item
from adventure.constants import ANSIBackgroundColours, ANSIBackgroundTextColours, ANSITextColours, Rarities
from adventure.helpers import escape, is_dev, smart_embed
