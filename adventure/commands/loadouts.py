...
from adventure.helpers import escape, smart_embed
...
# -*- coding: utf-8 -*-
"""
Loadout commands for GeneralsRPG.
"""
...
class LoadoutCommands:
    @commands.hybrid_command(name="loadout save")
    async def loadout_save(self, ctx: commands.Context, name: str):
        """Save your current equipment as a loadout."""
        c = await Character.from_json(ctx, self.config, ctx.author, self._daily_bonus)
        c.data.setdefault("loadouts", {})[name] = c.backpack.copy()
        await self.config.user(ctx.author).set(await c.to_json(ctx, self.config))
        return await smart_embed(ctx, _("Saved loadout {name}.").format(name=name))
