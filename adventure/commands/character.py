"""Character command group implementation moved into adventure.commands.

This module contains the implementation moved from the top-level shim so
`from adventure.commands.character import *` succeeds during the refactor.
"""
# -*- coding: utf-8 -*-
import logging
import time
...
# Ensure commonly-used decorator shims exist on the imported `commands` object
# so class-body decorators like @commands.hybrid_command don't raise at import
# time in minimal test environments.
if not hasattr(commands, "hybrid_command"):
    def _make_command_decorator(*dargs, **dkwargs):
        """Return a decorator that converts a function into a CommandShim.

        The CommandShim is callable (for runtime invocation) and exposes an
        `.autocomplete(param)` method used as a decorator at import-time to
        attach autocomplete handlers. This provides the minimal interface the
        codebase expects during collection without the full Red runtime.
        """
            ...
            class CommandShim:
                ...
                def autocomplete(self, param_name):
                    def _auto_decor(handler):
                        # attach the handler to both the shim and underlying
                        # function so code accessing either will find it.
...
if not hasattr(commands, "bot_has_permissions"):
...
# Ensure Context and Cog exist for annotations and isinstance checks during import
...
from adventure.converters import EquipableItemConverter, EquipmentConverter, SkillConverter
from adventure.helpers import ConfirmView, _title_case, escape, smart_embed
from adventure.menus import BaseMenu, SimpleSource
...
class CharacterCommands(AdventureMixin):
            ...
            if isinstance(item, list):
                ...
                for current_item in c.get_current_equipment():
                    if item.name.lower() in current_item.name.lower():
                        await c.unequip_item(current_item)
                        msg = _("{author} removed the {current_item} and put them into their backpack.").format(
                            author=escape(ctx.author.display_name), current_item=current_item
                        )
    ...
    @commands.hybrid_command()
    def equip(self, ctx: commands.Context, *, item: EquipableItemConverter):
        """Import-time safe no-op replacement for equip used during tests.

        The full implementation requires the Red runtime and async behaviour.
        For import-time stability and test collection we provide a minimal
        synchronous placeholder that does nothing.
        """
        return None
